...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `MOTOR_WAIT_TIMEOUT_MS` | 5000 | 等待电机到位的超时, 超时后继续执行并在串口打印 `waitForArrival timeout` |
| `SCAN_SETTLE_MS` | 100 | 扫描臂到位后读取颜色前的等待 |
| `SCAN_SAMPLES` | 3 | 每个色块读 3 次取中值; 设为 1 恢复单次读取 |
| `TURN_OVERSHOOT` | 0 | 拧层时先多转再回退的角度 (MindCuber 原版为 57); 0 为关闭 |

### 主机端测试

颜色识别、色块校验和求解器可以在 PC 上测试, 不需要连接硬件:

```
make test
```

测试使用 `fakedata.h` 中实测的扫描数据, 以及随机打乱的魔方。
