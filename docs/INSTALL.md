...
3. **（可选，一般不需要）白平衡校准。** 在 LCD 显示 `Insert cube...` 时**按一下“上”键**，LCD 显示 `Cal White:` / `White center up`。这时放入魔方，让**白色中心块朝上**。机器人检测到魔方约 1.5 秒后，扫描臂会移到中心读取 16 次取平均。成功时显示 `Cal White OK` 和三个读数，结果保存在 EEPROM 中，断电后仍然有效；读数过暗时显示 `Cal failed`，保留原来的值。之后按提示取出魔方即可。20 秒内没放入魔方会自动取消。从未校准过时，程序不做白平衡，这也是本项目调试和测试时使用的状态。
...
如果 3 轮扫描都识别失败，机器人会直接转到 `Remove cube...`，串口打印 `Scan error...`。可以检查光线条件，或者参考 readme 调整颜色传感器的位置。
...
**白平衡校准显示 `Cal failed` 或 `Cal canceled`**
- `Cal failed`：读数过暗，通常是扫描臂没有移到魔方中心上方，或者传感器离魔方太远。请检查扫描臂的位置。
- `Cal canceled`：按下“上”键后 20 秒内没有检测到魔方。请检查超声波传感器。

旧版本程序的校准要等到某个通道读数超过 250 才结束，而白色的实际读数只有 170~240，所以会一直卡在 `Cal White...`，扫描臂也会一直停在中心不动。如果你遇到过这种情况，请更新到新版程序。
